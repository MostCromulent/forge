...
import type { DeckCard, DeckDetails, DeckSummary, Seat, Send } from './protocol';
...
type SortKey = 'name' | 'colors' | 'formats' | 'size' | 'legal';
...
let cardFormat = 'any';
let legalOnly = false;
let cardFormats: string[] = [];
let timer = 0;
let details: DeckDetails | null = null;
let isHost = true;

export function openDeckFinder(index: number, seat: Seat, model: Model, sendFn: Send): void {
  seatIndex = index;
  send = sendFn;
  decks = model.decks ?? [];
...
export function onDeckDetails(deck: DeckDetails): void {
  if (overlay) {
    details = deck;
    renderPanel();
  }
...
export function onDecks(list: DeckSummary[] | undefined, formats: string[] | undefined): void {
  decks = list ?? [];
...
export function deckFinderOpen(): boolean {
  return !!overlay;
}

export function closeDeckFinder(): void {
  overlay?.remove();
  overlay = null;
  details = null;
}
...
function build(): void {
  ...
  q(back, '.dk-close').onclick = closeDeckFinder;
  q(back, '.cancel').onclick = closeDeckFinder;
  q(back, '.use').onclick = useChosen;
  const sortBy = q<HTMLSelectElement>(back, '.sort-by');
  sortBy.onchange = () => { sort = sortBy.value as SortKey; renderResults(); };
  const net = q(back, '.get-net');
  // Core asks which category through a dialog on the host's screen, so only the host can answer it
  net.hidden = !isHost;
  net.onclick = () => send({ t: 'netDecks' });
  const formatBy = q<HTMLSelectElement>(back, '.format-by');
  formatBy.onchange = () => { cardFormat = formatBy.value; renderResults(); };
  const legal = q<HTMLInputElement>(back, '.legal-only input');
  legal.onchange = () => { legalOnly = legal.checked; renderResults(); };
  const find = q<HTMLInputElement>(back, '.find');
...
function sources(): [string, number][] {
...
function renderFacets(): void {
  if (!overlay) return;
  const row = q(overlay, '.facets');
  row.replaceChildren(...sources().map(([id, count]) => {
    const b = document.createElement('button');
    b.className = 'facet';
    b.textContent = id === 'all' ? 'All sources' : id;
    b.append(Object.assign(document.createElement('span'), { className: 'dk-count', textContent: String(count) }));
...
function renderResults(): void {
  ...
  const list = matching();
  q(overlay, '.shown').textContent = list.length === decks.length
    ? `${decks.length} decks`
    : `${list.length} of ${decks.length} decks`;
  const rows = q(overlay, '.dk-hits');
  if (!list.length) {
...
function row(d: DeckSummary): HTMLElement {
...
const pips = (colors: string | undefined): string => [...(colors ?? '')].map(c => `<i class="pip pip-${c}">${c}</i>`).join('');
...
function renderPanel(): void {
  ...
  const panel = q(overlay, '.dk-chosen');
  q<HTMLButtonElement>(overlay, '.use').disabled = !chosen;
  if (!chosen) {
...
const CURVE_PX = 42;
const bar = (n: number, i: number, buckets: number, tallest: number): string => {
...
const section = (groups: DeckDetails['main']): string => groups.map(g => `
  <div class="group">
    <h4>${g.heading}<span>${g.cards.reduce((n, c) => n + c.count, 0)}</span></h4>
    ${g.cards.map(line).join('')}
  </div>`).join('');

const sideboard = (cards: DeckCard[]): string => (cards.length ? `
  <div class="group">
    <h4>Sideboard<span>${cards.reduce((n, c) => n + c.count, 0)}</span></h4>
    ${cards.map(line).join('')}
  </div>` : '');

const line = (c: DeckCard): string => `<div class="dk-line" data-image="${c.image}"><span class="n">${c.count}</span>${c.name}</div>`;

/** Hovering a card in the list shows it, the way hovering one on the table does. */
function peeking(panel: HTMLElement): void {
  const root = overlay;
  if (!root) return;
  const peek = q(root, '.deck-peek');
  const img = q<HTMLImageElement>(peek, 'img');
