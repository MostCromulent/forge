...
export function renderDialogs(model: Model, send: Send, schedule: () => void): void {
  ...
  if (req.id === shownId) {
    for (const c of layer.querySelectorAll<HTMLElement>('.card[data-key]')) {
...
function wrap(dlg: HTMLElement, at?: { x: number; y: number } | null): HTMLElement {
...
function build(req: Request, model: Model, answer: Answer): HTMLElement {
  ...
  switch (req.kind) {
    ...
    default: {
      ...
      dlg.append(actions(button('OK', true, () => answer(unknown.default))));
      return wrap(dlg);
    }
  }
}
...
function choices(dlg: HTMLElement, req: ChoicesRequest, model: Model, answer: Answer): HTMLElement {
  const reveal = req.kind === 'reveal';
  const picked = new Set<number>(reveal ? [] : req.selected);
  ...
  return wrap(dlg, req.atX || req.atY ? { x: req.atX ?? 0, y: req.atY ?? 0 } : null);
}

function order(dlg: HTMLElement, req: OrderRequest, model: Model, answer: Answer): HTMLElement {
...
function manipulate(dlg: HTMLElement, req: ManipulateRequest, model: Model, answer: Answer): HTMLElement {
  ...
  const row = (i: number, list: number[], pos: number) => {
...
function option(dlg: HTMLElement, req: OptionRequest, model: Model, answer: Answer): HTMLElement {
  if (req.card) {
...
function text(dlg: HTMLElement, req: TextRequest, answer: Answer): HTMLElement {
...
function distribute(dlg: HTMLElement, req: DistributeRequest, model: Model, answer: Answer): HTMLElement {
...
function sideboard(dlg: HTMLElement, req: SideboardRequest, model: Model, answer: Answer): HTMLElement {
  const inMain = [...req.main];
  const mainList = el('div', 'sb-list');
  const sideList = el('div', 'sb-list');
  const mainHead = el('h4');
  const sideHead = el('h4');
  const row = (i: number, count: number, arrow: string, move: () => void) => {
