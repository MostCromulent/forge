...
export function openOptions(onConcede: () => void): void {
  ...
  q(back, '.close').onclick = closeOptions;
  q(back, '.search').addEventListener('input', refreshOptions);
  back.onmousedown = e => {
    if (e.target === back) closeOptions();
  };
  const concede = q(back, '.concede');
  ...
  concede.onclick = () => {
    ...
    (concede.lastChild as ChildNode).textContent = 'Confirm concede';
  };
...
export function refreshOptions(): void {
  ...
  if (!dialog) return;
  const query = q<HTMLInputElement>(dialog, '.search').value.trim().toLowerCase();
  const rows = q(dialog, '.rows');
...
function cssControl(def: SettingDef, value: string): HTMLElement {
...
function playmatControl(def: SettingDef, value: string): HTMLElement {
  const wrap = document.createElement('div');
