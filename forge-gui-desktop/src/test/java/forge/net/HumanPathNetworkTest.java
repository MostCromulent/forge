...
public class HumanPathNetworkTest implements IHasForgeLog {
    ...
    /**
     * Four human-controlled players on Commander precons — the shape the delta-sync
     * ConcurrentModificationException was reported against. Three client processes and
     * multi-coloured precons put far more traffic through the freeze brackets and the
     * shared tracker than the two-player minimal-deck game above.
     */
    @Test(timeOut = 900000, description = "Four-player Commander game played through the human input path")
    public void playsFourPlayerCommanderThroughTheHumanInputPath() {
        skipUnlessStressTestsEnabled();
        ...
        netLog.info("Human-path 4p Commander result: {}", result.toSummary());
