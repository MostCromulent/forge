...
public class FGameClient implements IToServer, IHasForgeLog {
    ...
    private class LobbyUpdateHandler extends ChannelInboundHandlerAdapter {
        ...
        public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception {
            if (msg instanceof LobbyUpdateEvent event) {
            ...
            } else if (msg instanceof EventCreatedEvent event) {
                for (final ILobbyListener listener : lobbyListeners) {
                ...
                return;
            } else if (msg instanceof SelectEventForMatchEvent event) {
                for (final ILobbyListener listener : lobbyListeners) {
                    listener.selectEventForMatch(event.getEventId(), event.isDeckConformance());
                }
                return;
            } else if (msg instanceof DraftAutoPickedEvent event) {
                for (final ILobbyListener listener : lobbyListeners) {
                    listener.draftAutoPicked(event.getSeatIndex(), event.getCard(), event.getPickNumber());
                }
                return;
                ...
                for (final ILobbyListener listener : lobbyListeners) {
                    listener.draftSeatPicked(event.getSeatIndex(), event.getPickNumber(),
                            event.getSeatQueueDepths());
                }
                return;
            }
            super.channelRead(ctx, msg);
