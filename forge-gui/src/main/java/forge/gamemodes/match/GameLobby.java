...
public abstract class GameLobby implements IHasGameType {
    ...
    public String getCardPool() {
        return data.getCardPool();
    }

    public void setCardPool(final String name) {
        if (!Objects.equals(name, data.getCardPool())) {
            data.setCardPool(name);
            updateView(true);
        }
    }
    ...
    public Runnable startGame() {
        ...
        // The flag decides sides under Archenemy; a seat's own team is set only when the role moves
        final boolean archenemyTeams = variantTypes.contains(GameType.Archenemy);
        ...
        for (final LobbySlot slot : activeSlots) {
            ...
            if (autoGenerateVariant != null) {
                ...
                // The generated deck replaces the seat's cards, not the planes and schemes it brought
                if (slotDeck != null) {
                    for (final DeckSection section : List.of(DeckSection.Planes, DeckSection.Schemes)) {
                        if (slotDeck.has(section)) {
                            deck.putSection(section, slotDeck.get(section));
                        }
                    }
                }
            }
            ...
            rp.setTeamNumber(archenemyTeams ? (isArchenemy ? 0 : 1) : team);
    ...
    public final static class GameLobbyData implements Serializable {
        ...
        /** A GameFormat name every deck must be legal in, or null for none. Read by the web lobby only. */
